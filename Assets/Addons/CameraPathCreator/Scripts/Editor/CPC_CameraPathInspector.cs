...
public class CPC_CameraPathInspector : Editor
{
    ...
    void DrawNextHandle(int i)
    {
        if (i < t.points.Count - 1 || loopedProperty.boolValue)
        {
            ...
            if (handlePositionMode == CPC_EManipulationModes.Free)
            {
                ...
                var fmh_570_97_638687417880117010 = Quaternion.identity; posNext = Handles.FreeMoveHandle(t.points[i].position + t.points[i].handlenext, size, Vector3.zero, Handles.SphereHandleCap);
#else
    ...
    void DrawPrevHandle(int i)
    {
        if (i > 0 || loopedProperty.boolValue)
        {
            ...
            {
#if UNITY_5_5_OR_NEWER
                var fmh_622_97_638687417880135410 = Quaternion.identity; posPrev = Handles.FreeMoveHandle(t.points[i].position + t.points[i].handleprev, 0.1f * HandleUtility.GetHandleSize(t.points[i].position + t.points[i].handleprev), Vector3.zero, Handles.SphereHandleCap);
#else
    ...
    void DrawWaypointHandles(int i)
    {
        if (Tools.current == Tool.Move)
        {
            ...
            {
#if UNITY_5_5_OR_NEWER
                var fmh_678_68_638687417880206720 = (Tools.pivotRotation == PivotRotation.Local) ? t.points[i].rotation : Quaternion.identity; pos = Handles.FreeMoveHandle(t.points[i].position, HandleUtility.GetHandleSize(t.points[i].position) * 0.2f, Vector3.zero, Handles.RectangleHandleCap);
#else
